	...
	if (message.member !== null) {
		if (!message.member.user.bot) {
			if (cleanMessage.startsWith('!snax')) {
				...
				if (command.charAt(0) !== ' ') { // Confirm if there is a whitespace
					message.reply(utils.createEmbed(
						[':warning: Add a whitespace after ***!snax*** to run my commands :blush:']
					));
				} else { //It is a valid command!
					command = command.trim(); // Clean again
					//If it is a stats command
					if (command.toLowerCase().startsWith('stats')) { // Stats command
						utils.processStatsCommand(message, command, pool, new Discord.RichEmbed());
					} else if (command.toLowerCase().startsWith('profile')) { // Profile command
						utils.processProfileCommand(message, command, pool, guild, new Discord.RichEmbed());
					} else if (command.toLowerCase().startsWith('help')) { // Help command
						utils.processHelpCommand(message, 'all');
					} else if (config.sounds.indexOf(command) !== -1 || command.indexOf('sounds') !== -1) { // Sound command
						utils.processSoundCommand(message, command);
					} else if (message.author.username == 'Arctic Monkey' && command.toLowerCase().startsWith('podium') && message.channel.name === 'snax-bonobot') { //If it is a podium command
						utils.logTopOfTheMonth(pool, guild, false, new Discord.RichEmbed(), new Discord.RichEmbed());
					} else if (message.author.username == 'Arctic Monkey' && command.toLowerCase().startsWith('meme') && message.channel.name === 'snax-bonobot') { //If it is a meme command
						utils.logMemes(guild, new Discord.RichEmbed());
					} else if (command.toLowerCase().startsWith('club')) { // SNAX club command to get info
						utils.processSnaxClubCommand(message, command, pool, new Discord.RichEmbed());
					} 
			...
			} else if (cleanMessage == '.dailies') { // Dailies command
				utils.processDailiesCommand(message, pool);
			} else if (cleanMessage.startsWith('.rep ')) { // Reputation command
				utils.processRepCommand(message, pool);
			} else if (cleanMessage.startsWith('.race')) { // Race command
				utils.raceGame(guild, message, Discord);
			} else if (cleanMessage.startsWith('.hangman')) { // Hangman command
				utils.hangmanGame(guild, message, Discord);
			}  else if (cleanMessage.startsWith('.fight')) { // SNAX CLUB fight command
				utils.snaxClubGame(guild, message, Discord, new Discord.RichEmbed());
			} else if (cleanMessage.startsWith('.stop')) { // Stop command
				utils.stopPlayingSound(message);
			} else if (cleanMessage.startsWith('.playing')) { // Playing command
				utils.processPlayingCommand(message, guild, new Discord.RichEmbed());
			} else if (cleanMessage.startsWith('.')) { // Role command
				utils.processRoleCommand(message, guild);
			} else { // Register message stats
...
client.on('voiceStateUpdate', (oldMember, newMember) => {
	...
	var queryParams = [
		...
		newMember.user.displayAvatarURL.replace('size=2048', 'width=60&height=60'), // avatar
				...
				'IS NOT NULL ORDER BY (year, week) DESC LIMIT 1',
				  ...
				  if (shouldAbort(err)) {
					return;
							...
							endTimestamp = (newUserSelfMute ? endTimestamp.muted_timestamp : endTimestamp.unmuted_timestamp) + '';
...
client.on('presenceUpdate', (oldMember, newMember) => {
	if (newMember.presence.game !== null && newMember.presence.game.streaming !== null) {
		if (oldMember.presence.game === null || !oldMember.presence.game.streaming) {
			if (newMember.presence.game.streaming) {
				utils.logStreamMessage(guild, newMember, new Discord.RichEmbed());
			}
		}
	}
});
