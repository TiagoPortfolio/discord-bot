...
module.exports = {
    ...
    if (
      ...
      if (sound.indexOf("help") !== -1) {
        ...
        if (typeof message.member.voiceChannel !== "undefined") {
          ...
          message.reply(createEmbed(config.soundErrorDescriptions));
        }
      }
    }
  },
  ...
  processStatsCommand: function(message, command, pool, richEmbed) {
    ...
    if (paramsValues.length !== 0) {
      if (paramsValues[0] !== -1) {
        if (
          ["month", "all_time", "week", "last_week"].indexOf(
            paramsValues[0]
          ) !== -1
        ) {
          queryLabel = paramsValues[0];
        } else if (
          ["text", "voice", "dailies", "rep"].indexOf(
            paramsValues[0]
          ) !== -1
        ) {
    ...
    if (queryLabel === "month") {
      ...
      queryParams.push(
        getWeekOfYear(new Date()) - 1 < 1
          ? getISOWeeks(new Date().getFullYear())
          : getWeekOfYear(new Date()) - 1
      ); // Last Week
      query += " week = $1 GROUP BY discord_id, username ";
    }
    ...
    pool.query(query, queryParams, (err, res) => {
      if (err) {
        console.log(err);
        message.reply(createEmbed([":boom: An error occurred!"]));
      } else {
        if (res.rows.length === 0) {
          message.reply(createEmbed([":x: No stats returned!"]));
        } else {
          // Remove champion members from stats
          if (!globalMembers) {
            // Remove champion members from stats
            res.rows = res.rows.filter(function(user) {
              var member = guild.members.get(
                user.discord_id.toString()
              );
              if (typeof member !== "undefined") {
                var userRole = module.exports.getMemberRole(
                  member
                );
                return (
                  userRole.name != "Champion" &&
          ...
          if (pointsType != "dailies" && pointsType != "rep") {
            res.rows.forEach(function(user) {
              if (pointsType == "text") {
                user.points = calculateTextPoints(
                  user.valid_messages_count,
                  user.valid_characters_count,
                  user.dailies
                );
              } else if (pointsType == "voice") {
                user.points = calculateVoicePoints(
                  user.seconds_unmuted,
                  user.seconds_muted,
                  user.voice_points,
                  user.dailies
                );
            ...
            if (pointsType == "dailies") {
              ...
              res.rows.sort(function(a, b) {
                return (
                  parseInt(b.dailies) - parseInt(a.dailies)
                );
              });
              ...
              res.rows.sort(function(a, b) {
                return (
                  parseInt(b.reputation) -
                  parseInt(a.reputation)
                );
              });
            }
          }
  ...
  processProfileCommand: function(message, command, pool, guild, richEmbed) {
    ...
    if (typeof message.mentions.users.first() === "undefined") {
      ...
      if (command === "") {
        ...
        typeof parseInt(discordID) === "number" &&
    ...
    var query =
      "SELECT discord_id, username, SUM(messages_count) as messages_count, SUM(valid_messages_count) as valid_messages_count, " +
    ...
    if (params["discordID"]) {
      ...
      query +=
        " AND discord_id = " + (queryLabel !== "month" ? "$2" : "$1");
    } else if (params["username"]) {
      queryParams.push(username);
      query +=
        " AND LOWER(username) iLIKE " +
        (queryLabel !== "month" ? "$2" : "$1");
    }
          ...
          if (shouldAbort(err)) {
            ...
            if (author) {
              var user = {
                ...
                avatar: message.author.displayAvatarURL.replace(
                  "size=2048",
                  "width=60&height=60"
                ),
                messages_count: 0,
  ...
  processRoleCommand: function(message, guild) {
    ...
    if (
      ...
      let foundRole = guild.roles.find(
        role => role.name.toLowerCase() == newRole
      );
      ...
      if (foundRole) {
        if (config.availableRoles.indexOf(newRole) === -1) {
          message.reply(
            "you can't add **" +
              foundRole.name +
              "** role to yourself! :thinking:"
          );
        } else {
          // Check if guild member already has this role
          let memberRole = member.roles.find(
            role => role.name.toLowerCase() == newRole
          );

          if (memberRole) {
            message.reply(
              "you already have **" + foundRole.name + "** role!"
            );
          } else {
            member.addRole(foundRole);
            message.reply(
              "you now have **" + foundRole.name + "** role!"
            );
          }
        }
      }
  ...
  processDailiesCommand: function(message, pool) {
    ...
    var queryParams = [
      ...
      message.author.displayAvatarURL.replace(
        "size=2048",
        "width=60&height=60"
      ), // avatar
    ...
    pool.connect((err, client, done) => {
      const shouldAbort = err => {
        if (err) {
          console.error(
            "Error in profile transaction in dailies",
            err.stack
          );
          client.query("ROLLBACK", err => {
            if (err) {
              console.error(
                "Error rolling back client in dailies",
                err.stack
              );
            }
            // Release the client back to the pool
        ...
        client.query(
          ...
          (err, res) => {
            if (shouldAbort(err)) {
              ...
              var dailiesTimestamp = new Date(
                new Date(
                  res.rows[0].dailies_timestamp + ""
                ).getTime() +
                  60 * 60 * 24 * 1000
              );
              var totalSeconds = Math.floor(
                (dailiesTimestamp - dateUTC) / 1000
              );
            ...
            if (verifyTimestamp && time.hours >= 0) {
              message.channel.send(
                "**<:snax:230458568497692673>  |  " +
                  ...
                  ", your daily <:kfc:296050563374776320> SNAX points refreshes in " +
                  time.hours +
  ...
  processRepCommand: function(message, pool) {
    ...
    var repLimit =
      config.reputationLimit[
        module.exports.getMemberRole(message.member).name
      ];
    var responseMessage = "";
        ...
        client.query(
          ...
          (err, res) => {
            if (shouldAbort(err)) {
              ...
              var dailiesTimestamp = new Date(
                new Date(
                  res.rows[0].reputation_timestamp + ""
                ).getTime() +
                  60 * 60 * 24 * 1000
              );
              var totalSeconds = Math.floor(
            ...
            if (
              reputationSent.length >= repLimit &&
              verifyTimestamp &&
              time.hours >= 0
            ) {
              message.channel.send(
                "**<:snax:230458568497692673>  |  " +
                  message.author.username +
                  ", you can award more <:cookie:285277471094472716> reputation in " +
                  time.hours +
                  (time.hours != 1 ? " hours" : " hour") +
                  ", " +
                  time.minutes +
              ...
              // User has reputation to give
              if (
                typeof message.mentions.users.first() ===
                "undefined"
              ) {
                ...
                message.channel.send(
                  "**<:snax:230458568497692673>  |  " +
                    message.author.username +
                    ", you need to mention at least one user to award him with <:cookie:285277471094472716> reputation!** (type: `.rep help` for help)"
                );
              ...
              users = users.filter(function(user) {
                ...
                if (user.id == message.author.id) {
                  errorMessages[0] =
                    "**<:snax:230458568497692673>  |  " +
                    message.author.username +
                    ", did you really expect to be able give <:cookie:285277471094472716> reputation to yourself?! :bell: Shame! :bell:**\n";
                } else if (user.bot) {
                  // Can't give rep to bots
                  if (user.id == "352554272065585152") {
                    //snax-bonobot ID
                    errorMessages[1] =
                      "**<:snax:230458568497692673>  |  " +
                      message.author.username +
                      ", I know you love me but my <:cookie:285277471094472716> reputation is already at max level! :kissing_heart:**\n";
                  } else {
                    errorMessages[2] =
                      "**<:snax:230458568497692673>  |  " +
                      message.author.username +
                      ", you can't give <:cookie:285277471094472716> reputation to bots... but they still love you anyway! <:snaxhearts:296063806432608266>**\n";
                  }
                }
                return (
                ...
                if (reputationSent.length < repLimit) {
                  if (
                    ...
                    message.channel.send(
                      "**<:snax:230458568497692673>  |  " +
                        message.author.username +
                        ", you already gave <:cookie:285277471094472716> reputation to " +
                        user.username +
                            ...
                            if (shouldAbort(err)) {
                              ...
                              if (
                                ...
                                if (
                                  ...
                                  awardMessage +=
                                    "a <:cookie:285277471094472716> reputation point! (Reputation left: " +
                                    (repLimit -
                                      reputationSent.length) +
                                    " <:cookie:285277471094472716>)**\n";
  ...
  registerMessage: function(message, pool) {
    ...
    if (
      ...
      pool.query(
        ...
        [queryParams[0], queryParams[3], queryParams[4]],
        (err, res) => {
          if (err) {
            console.log(
              "Error on UPDATE valid messages and characters count for registerMessage: " +
                err
            );
          }
        }
  ...
  processMessageReaction: function(message) {
    ...
    if (random > 6) {
      ...
      if (cleanMessage.toLowerCase().indexOf("bonobo") !== -1) {
        message.channel
          .send("<:bonobojizz:320292029227728897>")
          .then(message => message.delete(10000));
        setTimeout(function() {
          message.react("320292029227728897");
        }, 10500);
      }

      if (cleanMessage.toLowerCase().indexOf("deez nuts") !== -1) {
        message.channel
          .send("Suck deez robotic nuts!\n:gear: :gear:")
          .then(message => message.delete(10000));
        setTimeout(function() {
          message.react("274322312453554176");
        }, 10500);
      }

      if (cleanMessage.toLowerCase().indexOf("poop") !== -1) {
        message.channel
          .send("<:kawaiipoop:296072782884896781>")
          .then(message => message.delete(10000));
        setTimeout(function() {
          message.react("296072782884896781");
        }, 10500);
      }

      if (
        cleanMessage.toLowerCase().indexOf("gay") !== -1 ||
        cleanMessage.toLowerCase().indexOf("kappapride") !== -1
      ) {
        message.channel
          .send("<:KappaPride:335587672653561856>")
          .then(message => message.delete(10000));
        setTimeout(function() {
          message.react("335587672653561856");
        }, 10500);
      }

      if (
        cleanMessage.toLowerCase().indexOf("kappa") !== -1 &&
        cleanMessage.toLowerCase().indexOf("kappapride") === -1
      ) {
        message.channel
          .send("<:kappa:230495935539576843>")
          .then(message => message.delete(10000));
        setTimeout(function() {
          message.react("230495935539576843");
        }, 10500);
      }

      if (cleanMessage.toLowerCase().indexOf("kfc") !== -1) {
        message.channel
          .send("<:kfc:296050563374776320>")
          .then(message => message.delete(10000));
        setTimeout(function() {
          message.react("296050563374776320");
        }, 10500);
      }

      if (cleanMessage.toLowerCase().indexOf("salt") !== -1) {
        message.channel
          .send("<:salt:278745703151435776>")
          .then(message => message.delete(10000));
        setTimeout(function() {
          message.react("278745703151435776");
        }, 10500);
      }
          ...
          message
            ...
            .then(reaction =>
              reaction.message
                .react("💗")
                .then(reaction =>
                  reaction.message.react("230476670421696523")
                )
            );
        } else {
          message
            .react("230476670421696523")
            .then(reaction =>
              reaction.message.react("284450372716855297")
            );
        }
      }
          ...
          message
            ...
            .then(reaction =>
              reaction.message
                .react("💗")
                .then(reaction =>
                  reaction.message.react("300551231829704705")
                )
            );
        } else {
          message
            .react("300551231829704705")
            .then(reaction =>
              reaction.message.react("284450372716855297")
            );
        }
      ...
      if (
        cleanMessage.toLowerCase().indexOf("rick") !== -1 &&
        cleanMessage.toLowerCase().indexOf("morty") !== -1
      ) {
        if (random == 3) {
          message.channel
            .send("<:TinyRick:331832938608066561>")
            .then(message => message.delete(10000));
          setTimeout(function() {
            message.react("331832938608066561");
          }, 10500);
        } else {
          message.channel
            .send("<:PickleRick:331833044023508995>")
            .then(message => message.delete(10000));
          setTimeout(function() {
            message.react("331833044023508995");
          }, 10500);
        }
      }
      ...
      if (cleanMessage.toLowerCase().indexOf("facepalm") !== -1) {
        message.channel
          .send("<:facepalm:278745692334456832>")
          .then(message => message.delete(10000));
        setTimeout(function() {
          message.react("278745692334456832");
        }, 10500);
      }
      ...
      if (cleanMessage.toLowerCase().indexOf("omar") !== -1) {
        if (random == 3) {
          message.channel
            .send("<:handsome_omar:325850491122614274>")
            .then(message => message.delete(10000));
          setTimeout(function() {
            message.react("325850491122614274");
          }, 10500);
        } else {
          message.channel
            .send("<:OmarPepe:324688036476616724>")
            .then(message => message.delete(10000));
          setTimeout(function() {
            message.react("324688036476616724");
          }, 10500);
        }
      }

      if (
        cleanMessage.toLowerCase().indexOf("dailure") !== -1 ||
        cleanMessage.toLowerCase().indexOf("dale") !== -1
      ) {
        message.channel
          .send("<:dailure_isthatso:325850313452158996>")
          .then(message => message.delete(10000));
        setTimeout(function() {
          message.react("325850313452158996");
        }, 10500);
      }

      if (cleanMessage.toLowerCase().indexOf("poky") !== -1) {
        message.channel
          .send("<:haha:334444953545605122>")
          .then(message => message.delete(10000));
        setTimeout(function() {
          message.react("334444953545605122");
        }, 10500);
      }

      if (cleanMessage.toLowerCase().indexOf("arctic") !== -1) {
        message.channel
          .send("<:arctic_wat:325850477109575691>")
          .then(message => message.delete(10000));
        setTimeout(function() {
          message.react("325850477109575691");
        }, 10500);
      }

      if (
        cleanMessage.toLowerCase().indexOf("donjay") !== -1 ||
        cleanMessage.toLowerCase().indexOf("don jay") !== -1
      ) {
        message.channel
          .send("<:coolstory_donjay:325849857321467916>")
          .then(message => message.delete(10000));
        setTimeout(function() {
          message.react("325849857321467916");
        }, 10500);
      }

      if (
        cleanMessage.toLowerCase().indexOf("feels good") !== -1 ||
        cleanMessage.toLowerCase().indexOf("feelsgood") !== -1
      ) {
        message.channel
          .send("<:feelsgoodman:284450372716855297>")
          .then(message => message.delete(10000));
        setTimeout(function() {
          message.react("284450372716855297");
        }, 10500);
      }

      if (
        cleanMessage.toLowerCase().indexOf("feels bad") !== -1 ||
        cleanMessage.toLowerCase().indexOf("feelsbad") !== -1
      ) {
        message.channel
          .send("<:feelsbadman:284396022682222592>")
          .then(message => message.delete(10000));
        setTimeout(function() {
          message.react("284396022682222592");
        }, 10500);
      }

      if (
        cleanMessage.toLowerCase().indexOf("vezzed") !== -1 ||
        cleanMessage.toLowerCase().indexOf("vexxed") !== -1 ||
        cleanMessage.toLowerCase().indexOf("vexed") !== -1
      ) {
        message.channel
          .send("V E <:veZzed:336635653259264010> E D")
          .then(message => message.delete(10000));
        setTimeout(function() {
          message
            .react("296029985779613701")
            .then(reaction =>
              reaction.message
                .react("336635653259264010")
                .then(reaction =>
                  reaction.message.react("296029498388774914")
                )
            );
        }, 10500);
      }
  ...
  processRandomBanana: function(message, pool, Discord) {
    ...
    const randomWords = ["mine", "yoink", "ilovebananas", "yeet"];
    const randomWord =
      ...
      channel
        ...
        .then(message => {
          const filter = message =>
            message.content.trim().toLowerCase() === randomWord;
  ...
  processPlayingCommand: function(message, guild, richEmbed) {
    ...
    guildMembers = guildMembers.filter(function(member) {
      return (
        ...
        config.blockedStatus.indexOf(member.presence.game.name) ===
          -1 &&
  ...
  processOwnCommand(message, command, pool) {
    ...
    if (typeof message.mentions.users.first() === "undefined") {
      ...
      if (user.id === message.author.id) {
        message.reply(
          createEmbed([
            ":warning: You have no freedom!"
          ])
        );
        return;
      } else if (message.mentions.users.length > 1) {
        message.reply(
          createEmbed([
            ":warning: You can only mention a single user!"
          ])
        );
        return;
      }
                                    ...
                                    (
                                      ...
                                      if (
                                        ...
                                        message.reply(
                                          createEmbed(
                                            [
                                              "<:bonobo:274325481833234432> Congratulations, **" +
                                                user.username +
                                                "** is now your bonobo oficially!"
                                            ]
                                          )
                                        );
                                      }
  ...
  processSnaxClubCommand(message, command, pool, richEmbed) {
    ...
    if (command.indexOf("help") !== -1) {
      ...
      return;
    } else if (command.indexOf("create")) {
      // Create character
      pool.connect((err, client, done) => {
        const shouldAbort = err => {
          if (err) {
                  ...
                  createEmbed([
                    ":warning: " +
                      message.mentions.users.first()
                        .username +
                      " still didn't join SNAX CLUB!"
                  ])
                );
                return;
              } else {
                // Character found
                let embed = createSnaxClubCharacterEmbed(
                  richEmbed,
                  res.rows[0].character_info
                );
                message.channel
                  .send({
                    embed
  ...
  registerMostPlayedGames: function(guild, richEmbed) {
    ...
    guildMembers.forEach(function(member) {
      ...
      for (let i = 0; i < config.mostPlayedGames.length; i++) {
        if (
          config.mostPlayedGames[i].game === member.presence.game.name
        ) {
          newGame = false;

          if (
            config.mostPlayedGames[i].users.usernames.indexOf(
              member.user.username
            ) === -1
          ) {
            config.mostPlayedGames[i].users.count += 1;
            config.mostPlayedGames[i].users.usernames.push(
              member.user.username
            );
          }
          break;
  ...
  logMostPlayedGamesOfTheDay: function(guild, richEmbed) {
    if (config.mostPlayedGames.length) {
      config.mostPlayedGames.sort(function(a, b) {
        ...
        return (
          b.users.count - a.users.count || b.gameValue - a.gameValue
        );
      });
      ...
      let channel = guild.channels.find(
        channel => channel.name === "bonobot-bonanza"
      );
      ...
      if (!channel) {
        ...
        let embed = createMostPlayedGamesEmbed(
          richEmbed,
  ...
  logTopOfTheMonth: function(
    ...
    pool.connect((err, client, done) => {
      const shouldAbort = err => {
        if (err) {
          console.error(
            "Error in transaction in logTopOfTheMonth",
            err.stack
          );
          client.query("ROLLBACK", err => {
            if (err) {
              console.error(
                "Error in rolling back client in logTopOfTheMonth",
                err.stack
              );
            }
            // Release the client back to the pool
      ...
      client.query("BEGIN", err => {
        ...
        client.query(query, [], (err, res) => {
          if (shouldAbort(err)) {
            ...
            if (res.rows.length === 0) {
              console.log(
                "ERROR: NO STATS RETURNED FROM logTopOfTheMonth"
              );
            } else {
              // Remove champion members from stats
              res.rows = res.rows.filter(function(user) {
              ...
              // Iterate through users from query result and calculate points and sort array after
              res.rows.forEach(function(user) {
                user.textPoints = calculateTextPoints(
                  user.valid_messages_count,
                  user.valid_characters_count,
                  user.dailies
                );
                user.voicePoints = calculateVoicePoints(
                  user.seconds_unmuted,
                  user.seconds_muted,
                  user.voice_points,
                  user.dailies
              ...
              res.rows.sort(function(a, b) {
                return (
                  parseInt(b.textPoints) -
                  parseInt(a.textPoints)
                );
              });
              ...
              res.rows.sort(function(a, b) {
                return (
                  parseInt(b.voicePoints) -
                  parseInt(a.voicePoints)
                );
              });
              ...
              for (var i = 1; i < dailiesTop.length; i++) {
                if (
                  dailiesTop[i - 1].dailies ==
                  dailiesTop[i].dailies
                ) {
                  continue;
                } else {
              ...
              for (var i = 1; i < reputationTop.length; i++) {
                if (
                  reputationTop[i - 1].reputation ==
                  reputationTop[i].reputation
                ) {
                  continue;
                } else {
                                                        ...
                                                        download(
                                                          ...
                                                          function() {
                                                            download(
                                                              ...
                                                              function() {
                                                                images(
                                                                  "./public/imgs/podium.png"
                                                                )
                                                                  .draw(
                                                                    images(
                                                                      "./public/imgs/avatars/second_text.png"
                                                                    ).size(
                                                                      100
                                                                    ),
                                                                    14,
                                                                  ...
                                                                  .draw(
                                                                    images(
                                                                      "./public/imgs/avatars/first_text.png"
                                                                    ).size(
                                                                      100
                                                                    ),
                                                                    129,
                                                                  ...
                                                                  .draw(
                                                                    images(
                                                                      "./public/imgs/avatars/third_text.png"
                                                                    ).size(
                                                                      100
                                                                    ),
                                                                    244,
                                                                ...
                                                                images(
                                                                  ...
                                                                  .draw(
                                                                    images(
                                                                      "./public/imgs/avatars/second_voice.png"
                                                                    ).size(
                                                                      100
                                                                    ),
                                                                    14,
                                                                  ...
                                                                  .draw(
                                                                    images(
                                                                      "./public/imgs/avatars/first_voice.png"
                                                                    ).size(
                                                                      100
                                                                    ),
                                                                    129,
                                                                  ...
                                                                  .draw(
                                                                    images(
                                                                      "./public/imgs/avatars/third_voice.png"
                                                                    ).size(
                                                                      100
                                                                    ),
                                                                    244,
                                                                  ...
                                                                  .then(
                                                                    function(
                                                                      ...
                                                                      channel
                                                                        .send(
                                                                          !mention
                                                                            ? "**Congratulations to :trophy: " +
                                                                                textTop[0]
                                                                                  .username +
                                                                                " :trophy: for being the most active user in :envelope: text channels in the last 4 weeks!!!**\n" +
                                                                                "Shoutout to **" +
                                                                                textTop[1]
                                                                                  .username +
                                                                                "** and **" +
                                                                                textTop[2]
                                                                                  .username +
                                                                                "** for reaching **2nd** and **3rd** place!\n"
                                                                            : "**Congratulations to " +
                                                                                " <@" +
                                                                                textTop[0]
                                                                                  .discord_id +
                                                                                "> " +
                                                                                "for being the most active user in :envelope: text channels in the last 4 weeks!!!**\n" +
                                                                                "Shoutout to " +
                                                                                " <@" +
                                                                                textTop[1]
                                                                                  .discord_id +
                                                                                          ...
                                                                                          .send(
                                                                                            !mention
                                                                                              ? "**Congratulations to :trophy: " +
                                                                                                  voiceTop[0]
                                                                                                    .username +
                                                                                                  " :trophy: for being the most active user in :microphone2: voice channels in the last 4 weeks!!!**\n" +
                                                                                                  "Shoutout to **" +
                                                                                                  voiceTop[1]
                                                                                                    .username +
                                                                                                  "** and **" +
                                                                                                  ...
                                                                                                  function() {
                                                                                                    ...
                                                                                                    dailiesMessage +=
                                                                                                      ...
                                                                                                      dailiesTop[0]
                                                                                                        .dailies /
                                                                                                        20 +
                                                                                                      " <:doublerainbow:335619664442818562> **Dailies**!\n";

                                                                                                    var reputationMessage =
                                                                                                      "- ";
                                                                                                    reputationTop.forEach(
                                                                                                      function(
                                                                                                        user
                                                                                                      ) {
                                                                                                        reputationMessage += mention
                                                                                                          ? "<@" +
                                                                                                            user.discord_id +
                                                                                                            "> "
                                                                                                          : user.username +
                                                                                                    ...
                                                                                                    reputationMessage +=
                                                                                                      ...
                                                                                                      reputationTop[0]
                                                                                                        .reputation +
                                                                                                      " <:cookie:285277471094472716> **Reputation**!\n";
  ...
  logMemes: function(guild, richEmbed) {
    var memeChannel = guild.channels.find(
      channel => channel.name === "memeville"
    );
    ...
    request("https://www.reddit.com/r/dankmemes/hot.json?limit=1", function(
      err,
      res,
      body
    ) {
      if (err) {
        console.error(
          "Error caught in logMemes inside request: " + err
        );
      } else {
        var body = JSON.parse(body);
        var meme =
          body.data.children[body.data.children.length - 1].data;
        ...
        memeChannel
          ...
          .then(messages => {
            ...
            botMessages = messages.filter(function(message) {
              if (
                message.author.username == "snax-bonobot" &&
                message.embeds.length
              ) {
                if (
                  typeof message.embeds[0].url !== "undefined"
                ) {
  ...
  logStreamMessage: function(guild, member, richEmbed) {
    ...
    let channel = guild.channels.find(
      channel => channel.name === "stream-addicts"
    );
    ...
    channel
      ...
      .then(messages => {
        messages.filter(function(message) {
          if (typeof message.embeds[0] !== "undefined") {
            if (
              ...
              member.user.username + " just started streaming!"
            ) {
        ...
        if (logStreamMessage) {
          ...
          https
            .get(encodeURI(member.presence.game.url), function(
              res
            ) {
              let rawData = "";
              res.on("data", chunk => {
              ...
              res.on("end", () => {
                try {
                  ...
                  streamImage = streamImage.replace(
                    /^\/\/?/i,
                    "https://"
                  );
    ...
    message.channel
      .send(
        ":race_car: The race is going to begin in **15 seconds**, type `.join` to join!" +
          ...
          racers.length +
          " " +
          players[racers.length - 1].avatar +
          " :** *" +
          message.author.username +
          "*"
      )
      .then(message => {
        const filter = message =>
          message.content.trim().toLowerCase() === ".join";
        // Create a message collector
        const collector = new Discord.MessageCollector(
          channel,
          filter,
        ...
        collector.on("collect", m => {
          ...
          if (racers.indexOf(m.author.username) === -1) {
            if (racers.length < players.length) {
              ...
              message.channel
                .send("Too late! Try to join next race...")
        ...
        collector.on("end", collected => {
          ...
          if (racers.length < 2) {
            message.channel.send(
              racers[0] +
                ", there's not enough players to race with <:feelsbadman:284396022682222592>"
            );
            config.raceInProgress = false;
            return;
          }
  ...
  hangmanGame: function(guild, message, Discord) {
    ...
    if (config.hangmanInProgress) {
      message.reply(
        "please wait for the current hangman game to be over!"
      );
      return;
    }
    ...
    let word =
      config.hangmanWords[
        Math.floor(Math.random() * (config.hangmanWords.length - 1))
      ];
    ...
    let drawBoard = function() {
      ...
      for (let i = 0; i < word.length; i++) {
        if (indicesFound.indexOf(i) !== -1) {
          board.word +=
            " :regional_indicator_" + word[i].toLowerCase() + ":";
        } else {
    ...
    message.channel
      ...
      .then(message => {
        // Create a message collector
        const filter = message =>
          message.content.search(/^[a-zA-Z]*$/) !== -1;
        const collector = new Discord.MessageCollector(
          channel,
          filter,
          { time: 120000 }
      ...
      channelMembers.forEach(function(member) {
        ...
        pool.connect((err, client, done) => {
          const shouldAbort = err => {
            if (err) {
              console.error(
                "Error in transaction updateVoicePoints",
                err.stack
              );
              client.query("ROLLBACK", err => {
                if (err) {
                  console.error(
                    "Error rolling back client updateVoicePoints",
                    err.stack
                  );
                }
                // Release the client back to the pool
  ...
  getMemberRole: function(member) {
    ...
    if (member !== "") {
      member.roles.filter(function(role) {
        var roleIndex = config.snaxRoles.findIndex(
          i =>
            i.name ==
            role.name.replace(/[^a-z0-9 ,.?!]/gi, "").trim()
        );
        if (roleIndex !== -1) {
          if (memberRole.name == "") {
            ...
            if (
              roleIndex <
              config.snaxRoles.findIndex(
                i => i.name == memberRole.name
              )
            ) {
              memberRole.name = role.name
                .replace(/[^a-z0-9 ,.?!]/gi, "")
                .trim();
              memberRole.emoji =
                config.snaxRoles[roleIndex].emoji;
            }
          }
        }
      });
  ...
  snaxClubGame: function(message, pool, Discord, richEmbed) {
    ...
    if (typeof challenger === "undefined") {
      ...
      message.channel.send(
        "**<:snax:230458568497692673> | <@" +
          message.author.id +
          ">, you need to mention at least one member to fight with!"
      );
      return;
          ...
          client.query(
            ...
            (err, res) => {
              if (shouldAbort(err)) {
                ...
                res.rows.length === 0 ||
                (res.rows.length === 1 &&
                  res.rows[0].player_discord_id !=
                    message.author.id)
              ) {
                // Challenger doesn't have a character created
                message.channel.send(
                  "**<:snax:230458568497692673>  | <@" +
                    message.author.id +
                    ">, you still need to create a character to fight!\nType ```!snax club create``` to create your character."
                );
                return;
              } else if (
                res.rows.length === 1 &&
                ...
                // Challenged doesn't have a character created
                message.channel.send(
                  "**<:snax:230458568497692673>  | <@" +
                    message.author.id +
                    ">, " +
                    challenger.username +
                    " still needs to create a character to fight!\n<@" +
                    challenger.id +
                    ">, if you want to create your character type ```!snax club create```"
                );
                return;
              } else {
                // Check if player still has a challenge open
                if (
                  utils.snaxClub.openChallenges.hasOwnProperty(
                    message.author.username
                  )
                ) {
                  message.channel.send(
                ...
                let embed = createSnaxClubFightEmbed(
                  richEmbed,
                  res.rows[0].character_info,
                  res.rows[1].character_info
                );
                message.channel.send({
                  embed
                });
                ...
                collector.on("collect", m => {
                  for (key in utils.snaxClub.openChallenges) {
                    if (
                      ...
                      client.query(
                        //Check if any player still has a battle active
                        ...
                        (err, res) => {
                          if (shouldAbort(err)) {
                            ...
                            message.reply(
                              createEmbed([
                                "You still have a battle to finish with" +
                                  (res.rows[0]
                                    .player1_discord_id ==
                                  m.author.id
                                    ? res
...
function createProfileEmbed(richEmbed, properties, type) {
  ...
  properties.valid_characters_count = parseInt(
    properties.valid_characters_count
  );
  ...
  richEmbed.setURL(
    encodeURI(
      "https://snax-bonobot.herokuapp.com/member/" + properties.username
    )
  );
  richEmbed.setColor(0x0074e8);
  richEmbed.setTitle(
    properties.username + "  |  " + role.emoji + " " + role.name
  );
  ...
  if (properties.avatar !== "") {
    richEmbed.setThumbnail(
      properties.avatar.replace("size=2048", "width=60&height=60")
    );
  }
  ...
  if (properties.username == config.topMembers.text) {
    ...
    richEmbed.addField(
      "**:envelope: Text Champion :trophy:**",
      "Most Active User in :envelope: Text Channels of " +
  ...
  if (properties.username == config.topMembers.voice) {
    ...
    date.setMonth(date.getMonth() - 1);
    richEmbed.addField(
      "**:microphone2: Voice Champion :trophy:**",
      "Most Active User in :microphone2: Voice Channels of " +
        date.toLocaleString("en-us", { month: "long" })
    );
  }
  ...
  if (config.topMembers.dailies.includes(properties.username)) {
    ...
    date.setMonth(date.getMonth() - 1);
    richEmbed.addField(
      "**<:doublerainbow:335619664442818562> Dailies Champion :trophy:**",
      "User with Most <:doublerainbow:335619664442818562> Dailies of " +
        date.toLocaleString("en-us", { month: "long" })
    );
  }
  ...
  if (config.topMembers.reputation.includes(properties.username)) {
    ...
    date.setMonth(date.getMonth() - 1);
    richEmbed.addField(
      "**<:cookie:285277471094472716> Reputation Champion :trophy:**",
      "User with Most <:cookie:285277471094472716> Reputation of " +
        date.toLocaleString("en-us", { month: "long" })
    );
  }

  if (
    properties.member.presence.game !== null &&
    !properties.member.user.bot &&
    properties.member.presence.game.name.length < 50 &&
    config.blockedStatus.indexOf(properties.member.presence.game.name) ===
      -1
  ) {
    richEmbed.addField(
      "Playing Right Now :joystick:",
      properties.member.presence.game.name
    );
  }

  // richEmbed.addField('SNAXKREW Rank ' + role.emoji, role.name, true);
  richEmbed.addField(
    "Text Points <:kfc:296050563374776320>",
    calculateTextPoints(
      properties.valid_messages_count,
      properties.valid_characters_count,
      properties.dailies
    ),
    true
  ...
  richEmbed.addField(
    "Voice Points <:kfc:296050563374776320>",
    calculateVoicePoints(
      properties.seconds_unmuted,
      properties.seconds_muted,
      properties.voice_points,
      properties.dailies
    ),
    true
  );
  richEmbed.addField(
    "Messages Sent :incoming_envelope:",
    properties.messages_count,
    true
  );
  richEmbed.addField(
    "Minutes Speaking :microphone2:",
    Math.floor(properties.seconds_unmuted / 60),
    true
  );
  // richEmbed.addField('Chars/Msgs Ratio :pencil:', (properties.characters_count / properties.valid_messages_count).toFixed(2), true);
  richEmbed.addField(
    "Characters Sent :keyboard:",
    properties.characters_count,
    true
  );
  richEmbed.addField(
    "Minutes Muted :mute:",
    Math.floor(properties.seconds_muted / 60),
    true
  );
  richEmbed.addField(
    "Dailies <:doublerainbow:335619664442818562>",
    properties.dailies / 20,
    true
  );
  richEmbed.addField(
    "Reputation <:cookie:285277471094472716>",
...
function createStatsEmbed(richEmbed, users, type, pointsType) {
  var emoji =
    pointsType == "text"
      ...
      ? ":microphone2:"
      : pointsType == "dailies"
      ? "<:doublerainbow:335619664442818562>"
      : pointsType == "rep"
      ? "<:cookie:285277471094472716>"
      : ":trophy:";
  ...
  if (pointsType !== "dailies" && pointsType !== "rep") {
    ...
    richEmbed.setTitle(
      emoji +
        " Users with Most " +
        (pointsType == "dailies" ? "Dailies" : "Reputation") +
        " of " +
        (type === "month"
          ? "the Month "
          : type === "all_time"
    ...
    richEmbed.setDescription(
      "*Based on SNAXKREW Members Activity" +
        (type === "month" ? " - Last 4 Weeks*" : "*")
    );
  }
  ...
  for (var i = 0; i < users.length; i++) {
    ...
    richEmbed.addField(
      podium + " " + users[i].username,
      pointsType == "dailies"
        ? users[i].dailies / 20 + " <:doublerainbow:335619664442818562>"
        : pointsType == "rep"
        ? users[i].reputation + " <:cookie:285277471094472716>"
        : users[i].points + " <:kfc:296050563374776320>",
      true
    );
  }
  // richEmbed.setFooter('snax-bonobot', 'http://www.snaxkrew.com/snaxkrew/imgs/snaxbanner.png');
...
function createMostPlayedGamesEmbed(richEmbed, gamesPlayed) {
  ...
  gamesPlayed.forEach((element, i) => {
    ...
    element.users.usernames.sort(function(a, b) {
      let usernameA = a.toLowerCase(),
        usernameB = b.toLowerCase();
...
function createPodiumEmbed(richEmbed, users, pointsType, image) {
  ...
  for (var i = 0; i < users.length; i++) {
    ...
    richEmbed.addField(
      ...
      (pointsType == "voice"
        ? users[i].voicePoints
        : users[i].textPoints) + " <:kfc:296050563374776320>",
      true
    );
...
function createMemeEmbed(richEmbed, properties) {
  ...
  richEmbed.setDescription(
    "<:teehee:335621295448260608> Hot Meme! <:teehee:335621295448260608>"
  );
...
function createStreamEmbed(richEmbed, member) {
  ...
  richEmbed.setDescription(
    "**:tv: " +
      ...
      " :tv:**\n<:feelsgoodman:284450372716855297> Watch the stream at " +
      encodeURI(member.presence.game.url)
  );
  richEmbed.setThumbnail(
...
function createSnaxClubCharacterEmbed(richEmbed, character) {
  ...
  richEmbed.addField(
    "**XP**",
    character.xp + " / " + character.level * xp,
    true
  );
...
function createSnaxClubFightEmbed(richEmbed, character1, character2) {
  ...
  richEmbed.addField(
    "**" +
      ...
      "**\nLevel: " +
      character1.level +
      (statWinner.level ? " :arrow_up:" : " :arrow_down:") +
      "\nHP: " +
      character1.hp +
      (statWinner.hp ? " :arrow_up:" : " :arrow_down:") +
      "\nAttack: " +
      character1.attack +
      (statWinner.attack ? " :arrow_up:" : " :arrow_down:") +
      "\nDefense: " +
      character1.defense +
      (statWinner.defense ? " :arrow_up:" : " :arrow_down:") +
      "\nDodge: " +
      character1.dodge +
      (statWinner.dodge ? " :arrow_up:" : " :arrow_down:"),
    true
  );
...
function calculateTextPoints(
  ...
  return (
    parseInt(
      (
        valid_messages_count * 5 +
        Math.min(
          valid_characters_count / 20,
          valid_messages_count * 4
        ) *
          2
      ).toFixed(0)
    ) + dailies
...
function calculateVoicePoints(
  ...
  return (
    parseInt(
      (
        Math.floor(seconds_muted / 60) * 0.5 +
